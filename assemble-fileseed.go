...
func NewFileSeed(dstFile string, srcFile string, index Index) (*FileSeed, error) {
	return &FileSeed{
		srcFile:   srcFile,
		seedIndex: newSeedIndex(index, CanClone(dstFile, srcFile)),
	}, nil
}
...
func (s *FileSeed) LongestMatchFrom(chunks []IndexChunk, startPos int) (int, int) {
	return s.longestMatchFrom(chunks, startPos, 0, 0, nil)
}
...
func (s *FileSeed) RegenerateIndex(ctx context.Context, n int, attempt int, seedNumber int) error {
	...
	s.seedIndex = newSeedIndex(index, s.canReflink)
	return nil
}
...
// access is only the write, the seed is another file than the target.
func (s *fileSeedSource) access() targetAccess {
	return targetAccess{writes: byteRange{s.offset, s.offset + s.length}}
}
