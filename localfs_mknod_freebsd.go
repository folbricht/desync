...
// Like Linux, FreeBSD has mknodat(2), so the parent directory is opened
// through the os.Root handle (which refuses to traverse any symlink escaping
// the root) and the node is created relative to that directory fd with a base
